        ...
        if (!logged) 
        {
            ...
            switch (choice) 
            {
                case 0:
                    ...
                    // 检查用户名是否已存在
                    it = find_if(users.begin(), users.end(), [&username](const User& user) {
                ...
                case 1:
                    ...
                    if (it != users.end() && it->password == inputPassword) {
                        cout << "登录成功！\n";
                        logged = 1;
                    } else {
                        cout << "用户名或密码错误。\n";
